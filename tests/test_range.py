...
def test_minute_range():
    ...
    next_datetime = datetime.now().replace(second=0, microsecond=0)
...
def test_day_range():
    ...
    next_datetime = datetime.now().replace(hour=0, minute=0, second=0, microsecond=0)
...
def test_weekday_number_range():
    ...
    next_datetime = datetime.now().replace(hour=10, minute=0, second=0, microsecond=0)
    while True:
...
def test_month_number_range():
    ...
    next_datetime = datetime.now().replace(day=1, hour=10, minute=0, second=0, microsecond=0)
    while True:
        if next_datetime.month in range(2, 9) and next_datetime.day == 1 and next_datetime > datetime.now():
            break
        next_datetime += timedelta(days=1)
    next_schedule = next(cron)  # type: datetime
    assert isinstance(next_schedule, datetime)
...
def test_month_name_range():
    ...
    next_datetime = datetime.now().replace(day=1, hour=10, minute=0, second=0, microsecond=0)
    while True:
        if next_datetime.month in range(3, 12) and next_datetime.day == 1 and next_datetime > datetime.now():
            break
        next_datetime += timedelta(days=1)
    next_schedule = next(cron)  # type: datetime
    assert isinstance(next_schedule, datetime)
...
def test_year_range():
    ...
    next_datetime = datetime.now().replace(month=2, day=1, hour=10, minute=0, second=0, microsecond=0)
    while True:
